...
export function generateFootprints(directory: string, output: string) {
  ...
  for (const library of readdirSync(directory).filter(f => f.endsWith('.pretty')).sort()) {
    ...
    const destination = join(output, 'native', library)
    ...
    for (const file of readdirSync(join(directory, library)).filter(f => f.endsWith('.kicad_mod')).sort()) {
      ...
      text += '/** '+doc+' */\nexport const '+exported+': Footprint = { id: '+JSON.stringify(id)+', file: fileURLToPath(new URL('+JSON.stringify('./native/'+library+'/'+file)+', import.meta.url)), ...'+JSON.stringify(footprintMetadata(tree))+' }\n\n'
      copyFileSync(join(directory, library, file), join(destination, file))
      count++
    }
    writeFileSync(join(output, name+'.ts'), text)
  }
  console.log(`Generated ${count} footprints in ${output}`)
}

export async function main(args: string[]) {
  if (args.length > 1) throw new Error('Usage: ts-kicad generate-footprints [KiCad-footprint-directory]')
  ...
  generateFootprints(directory, join(import.meta.dir, '../footprints'))
}
