...
export function writeDocs(schematic:string,output:string){
  ...
  if(existsSync(readme)&&readFileSync(readme,'utf8').includes('Generated from the native KiCad netlist.'))unlinkSync(readme)
...
export function checkPopulation(board:Node,parts:Part[]){
  ...
  const actual=children(board,'footprint').concat(children(board,'module')).filter(fp=>!child(fp,'attr').slice(1).some(a=>['dnp','exclude_from_bom'].includes(val(a)))).map(fp=>prop(fp,'Reference')).sort(order)
...
function mechanical(board:string,output:string){
  ...
  for(const type of ['step','stl'])run(['kicad-cli','pcb','export',type,'--force',...(type==='step'?['--subst-models']:['--board-only','--fuse-shapes']),'--output',join(output,basename(board,'.kicad_pcb')+'.'+type),...origin,board])
}
export function fabricate(boardPath:string,schematic:string,output:string,parts:Part[]){
  const board=parse(readFileSync(boardPath,'utf8')),name=basename(boardPath,'.kicad_pcb');mkdirSync(output,{recursive:true})
  try{
    checkPopulation(board,parts);checkErc(schematic)
    const report=join(output,name+'.drc.json');run(['kicad-cli','pcb','drc',boardPath,'--schematic-parity','--format','json','--output',report])
    const drc=JSON.parse(readFileSync(report,'utf8')),issues=[...drc.violations,...drc.schematic_parity,...drc.unconnected_items]
    if(issues.length)throw new Error(`PCB checks failed: ${issues.length} violations/unconnected items. See ${report}`)
    unlinkSync(report)
  }catch(error){
  ...
  withTemp(stage=>{
    ...
    mkdirSync(gerbers)
    run(['kicad-cli','pcb','export','gerbers','--output',gerbers+'/','--layers',layers.join(','),'--check-zones',boardPath])
    ...
    const positions=join(stage,name+'-pos.csv');run(['kicad-cli','pcb','export','pos','--output',positions,'--side','both','--format','csv','--units','mm','--exclude-dnp',boardPath])
...
export async function main(args:string[]){
  const {positionals,values}=cli({output:{type:'string'},'docs-only':{type:'boolean'}},args)
  if(positionals.length!==1)throw new Error('Usage: ts-kicad outputs project-directory|board.kicad_sch|board.kicad_pcb [--output directory] [--docs-only]')
  ...
  const schematic=target.replace(/\.kicad_pcb$/,'.kicad_sch'),board=schematic.replace(/\.kicad_sch$/,'.kicad_pcb'),output=resolve(values.output as string??join(dirname(target),'generated'))
  const parts=writeDocs(schematic,output)
  if(!values['docs-only']&&existsSync(board))fabricate(board,schematic,output,parts)
}
