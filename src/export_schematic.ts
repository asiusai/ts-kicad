import { planNetLabels, type NetLabel } from './net_labels'
...
import { inspect, readNetlist, run, cli, type ModelPart, type ModelPin } from './kicad_io'
...
function instance(card:Card,x:number,y:number,ref:string,rootId:string,sheetId:string,project:string,key:string):Node {
  ...
  result.push(N('instances',N('project',project,N('path','/'+rootId+'/'+sheetId,N('reference',ref),N('unit',A(card.unit))))))
  return result
}
...
export async function exportSchematic(entry:string|string[],output:string,sources:string[],options:ProjectOptions & {verify?:boolean;pdf?:boolean;layout?:LayoutMode}={}){
  ...
  for (const item of items) {
    ...
    const original = item.schema, suffix = createHash('sha256').update(JSON.stringify(item.pinTypes)).digest('hex').slice(0,8)
    ...
    for (const unit of children(lib, 'symbol')) {
      ...
      for (const pin of children(unit, 'pin')) { const type = item.pinTypes[val(child(pin,'number')[1])]; if(type) pin[1] = A(type) }
    }
  ...
  let powerIndex=0
  for(const item of items)sections.set(item.sheet||'Circuit',[...sections.get(item.sheet||'Circuit')??[],item])
  const outputs:{sheet:string;filename:string;id:string;tree:Node}[]=[],filenames=new Set<string>()
  for(const [sheet,parts]of sections){
    const id=uid(project+'/'+sheet),filename=project+'--'+sheet.replace(/[^A-Za-z0-9_-]/g,'_')+'.kicad_sch'
    if(filenames.has(filename))throw new Error('Section filename collision: '+sheet);filenames.add(filename)
    ...
    const drawing=createSheet(id,used,width,height,sheet)
    drawing.push(text(sheet,15,18,sheet+'/title',2.54))
    ...
    const drawLabel=(name:string,x:number,y:number,angle:number,key:string)=>{
      ...
      const lib=libraries.get(spec.powerSymbol)!,ground=name.startsWith('GND'),symbolId=uid(key+'/power')
      return N('symbol',N('lib_id',spec.powerSymbol),at(snap(x),snap(y)),N('unit',A(1)),N('in_bom',A('no')),N('on_board',A('no')),N('dnp',A('no')),N('uuid',symbolId),
        ...
        N('instances',N('project',project,N('path','/'+rootId+'/'+id,N('reference',ref),N('unit',A(1))))))
    }
    for(const {group:g,x:gx,y:gy}of packed.groups){
      const x=snap(gx+15),y=snap(gy+30),groupStart=drawing.length,members:string[]=[]
      ...
      for(const p of g.parts){
        ...
        const symbol=instance(card,px,py,refs.get(card.item.name)!,rootId,id,project,key);drawing.push(symbol);members.push(val(child(symbol,'uuid')[1]))
        for(const pin of card.geometry){
          ...
          drawing.push(w,l);members.push(val(child(w,'uuid')[1]),val(child(l,'uuid')[1]))
        }
      }
    ...
    outputs.push({sheet,filename,id,tree:drawing})
  }
  const root=createSheet(rootId,[],297,Math.max(210,42+Math.ceil(outputs.length/3)*36+35),project)
  root.push(text(project.toUpperCase(),15,16,project+'/title',3),text('Circuit sections - generated from TypeScript',15,23,project+'/subtitle',1.4))
  for(const [i,s]of outputs.entries()){
    const x=18+i%3*88,y=42+Math.floor(i/3)*36
  ...
  if(options.pdf){
    ...
    process.stdout.write(run(['kicad-cli','sch','export','pdf','-o',join(directory,basename(output,'.kicad_sch')+'.pdf'),output]))
  }
