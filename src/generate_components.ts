...
export async function main(args: string[]) {
  ...
  if (positionals.length > 1) throw new Error('Usage: ts-kicad generate-components [KiCad-symbol-directory]')
  const directory = positionals[0] ?? symbolDirectory(), output = join(import.meta.dir, '../components')
