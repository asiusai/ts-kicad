...
/** Download into staging, validate native pin mappings, then publish and sync the libraries. */
export async function importParts(ids: readonly string[], directory: string, download: Download = downloadPart) {
  ...
  for (const id of unique) if (existsSync(join(project, 'lib', id))) throw new Error(`Library lib/${id} already exists; preserving its edits. Use sync to refresh its bindings.`)
  ...
  try {
    ...
    // syncParts resolves model paths, upgrades legacy footprints, generates TS and updates KiCad tables.
...
export async function main(args: string[]) {
  if (args[0] && /\.(kicad_sch|xml)$/i.test(args[0])) return (await import('./import_schematic')).main(args)
  if (args.some((arg) => arg.startsWith('-'))) throw new Error('Usage: ts-kicad import C<number> [C<number> ...] [project-directory]')
  const ids = [...args]
  const directory = ids.length > 1 && !/^C\d+$/i.test(ids.at(-1)!) ? ids.pop()! : '.'
  await importParts(ids, directory)
}
